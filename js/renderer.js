...
function renderActivities(data) {
  ...
  const filter  = document.getElementById('actYearFilter');
  ...
  if (filter)  filter.style.display  = 'none';   // one card per year — no filter needed

  const lang = getLang();

  grid.innerHTML = data.map((act, i) => {
    const name     = lang === 'th' ? act.name_th : act.name_en;
    const cover    = driveUrl(act.cover_url || '');
    const albumUrl = act.album_url || '#';

    return `
    <a class="act-card reveal-up" href="${escAttr(albumUrl)}"
       target="_blank" rel="noopener noreferrer"
       style="--delay:${(i % 4) * 0.07}s; text-decoration:none">
      <div class="act-card-img-wrap">
        ${cover
          ? `<img class="act-card-img" src="${escAttr(cover)}" alt="${escAttr(name)}" loading="lazy">`
        ...
        <div class="act-album-overlay">
          ...
          <span data-th="ดูกิจกรรมทั้งหมด" data-en="View All Photos">${lang === 'th' ? 'ดูกิจกรรมทั้งหมด' : 'View All Photos'}</span>
        </div>
      </div>
      <div class="act-card-body">
        <span class="act-album-year">${escAttr(act.year || '')}</span>
        <h3 class="act-card-title"
            data-th="${escAttr(act.name_th)}"
            data-en="${escAttr(act.name_en)}">${escAttr(name)}</h3>
        <span class="act-album-link">
          <span data-th="ดูกิจกรรมทั้งหมด" data-en="View All Photos">${lang === 'th' ? 'ดูกิจกรรมทั้งหมด' : 'View All Photos'}</span>
